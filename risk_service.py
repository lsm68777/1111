...
import uvicorn
# import 구문들 다음에 바로 이것만 추가
# 기존 코드 다음에 이것만 추가:
os.environ['PYTHONIOENCODING'] = 'utf-8'
if hasattr(sys.stdout, 'reconfigure'):
    sys.stdout.reconfigure(encoding='utf-8', errors='ignore')
print("[Phoenix95] 인코딩 설정 완료")
...
print("[Phoenix95] logging 재설정 완료")
...
RISK_CONFIG = {
    ...
    "max_daily_loss": 0.2,               # 일일 최대 손실 20%
    "max_position_size": 0.02,            # 최대 포지션 크기 2%
    "max_positions": 5,                   # 최대 동시 포지션 5개
    "confidence_threshold": 0.75,         # 최소 신뢰도 75%
    
    # Kelly Criterion 설정
    "kelly_max_fraction": 0.02,           # Kelly 최대 2%
...
class RiskService:
    ...
    def __init__(self):
        ...
        logging.info(f"[SHIELD] Phoenix 95 Risk Service v{RISK_CONFIG['version']} 초기화 완료")
    ...
    async def start_background_tasks(self):
        """백그라운드 태스크 시작"""
        logging.info("[REFRESH] 백그라운드 태스크 시작")
        ...
        logging.info(f"[OK] {len(self.background_tasks)}개 백그라운드 태스크 시작됨")
